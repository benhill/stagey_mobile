...
var url = "http://www.gwahir.com:3000/api/project/" + currentWin.project_id + ".json";
...
var xhr = Ti.Network.createHTTPClient({
  onload: function(){
    ...
    if(project.is_favorite){      
      var make_favorite = new Icon(remove_fav_text, 'icons/favorite_24.png', '', project);
    }
    else{
      var make_favorite = new Icon(make_fav_text, 'icons/favorite_24.png', '', project); 
    }
    icons.push(make_favorite);

    var share = new Icon('Share', 'icons/share_24.png', '', project);
    icons.push(share);    

    iconsView = Ti.UI.createView({
      top:0,
      left:0,
      height:60,
      layout:'absolute'
    ...
    currentWin.add(projectScroll);
    currentWin.remove(spinner);
    currentWin.open();    
  },
  onerror: function(e) {
...
function toggleFavorite(){
  url = "http://www.gwahir.com:3000/api/toggle_favorite.json?project_id=" + currentWin.project_id + "&email=" + Ti.App.currentUser.email + "&password=" + Ti.App.userPassword;
  favXhr.open("GET", url);
  favXhr.send();  
}